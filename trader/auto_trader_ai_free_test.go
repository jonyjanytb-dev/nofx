...
func TestSortAIFreeDecisionsCloseFirstNoSameCycleFlip(t *testing.T) {
	...
	if len(out) != 3 { t.Fatalf("same-cycle BTC flip should be removed, got %+v", out) }
	if out[0].Action != "close_long" { t.Fatalf("close must be first: %+v", out) }
...
func TestNext15mCloseDelay(t *testing.T) {
	...
	if d != want { t.Fatalf("got %v want %v", d, want) }
}
...
func TestAIFreeRiskShrinksNotional(t *testing.T) {
	...
	if !adj.Adjusted || d.PositionSizeUSD >= 500 { t.Fatalf("expected risk shrink, got %+v decision=%+v", adj, d) }
	if adj.EstimatedRiskUSDT > 5.000001 { t.Fatalf("estimated risk exceeds hard max: %+v", adj) }
}

func TestAIFreeProtectionOptionalTP(t *testing.T) {
	if err := validateAIFreeProtectionPrices("open_long", 100, 95, 0); err != nil { t.Fatal(err) }
	if err := validateAIFreeProtectionPrices("open_short", 100, 105, 0); err != nil { t.Fatal(err) }
}

func TestCanTightenAIFreeStop(t *testing.T) {
	if !canTightenAIFreeStop("long", 95, 97) { t.Fatal("long stop should tighten upward") }
	if canTightenAIFreeStop("long", 95, 94) { t.Fatal("long stop must not loosen downward") }
	if !canTightenAIFreeStop("short", 105, 103) { t.Fatal("short stop should tighten downward") }
	if canTightenAIFreeStop("short", 105, 106) { t.Fatal("short stop must not loosen upward") }
}
...
func TestSameSymbolPositionExistsRegardlessOfSide(t *testing.T) {
	...
	if !sameSymbolPositionExists(positions, "BTCUSDT") { t.Fatal("same symbol must be occupied regardless of side") }
}
...
func TestAIFreeManagedCapitalCapsAggregateMargin(t *testing.T) {
	...
	if err!=nil{t.Fatal(err)}
	if adj.AllowedNotional>100.000001{t.Fatalf("managed-capital cap exceeded: %+v",adj)}
}

type fakeAIFreeTrader struct { positions []map[string]interface{} }

func (f *fakeAIFreeTrader) GetBalance() (map[string]interface{}, error) { return map[string]interface{}{"totalEquity":100.0,"availableBalance":100.0},nil }
func (f *fakeAIFreeTrader) GetPositions() ([]map[string]interface{}, error) { return f.positions,nil }
func (f *fakeAIFreeTrader) OpenLong(string,float64,int)(map[string]interface{},error){return map[string]interface{}{},nil}
func (f *fakeAIFreeTrader) OpenShort(string,float64,int)(map[string]interface{},error){return map[string]interface{}{},nil}
func (f *fakeAIFreeTrader) CloseLong(string,float64)(map[string]interface{},error){f.positions=nil;return map[string]interface{}{"orderId":int64(1)},nil}
func (f *fakeAIFreeTrader) CloseShort(string,float64)(map[string]interface{},error){f.positions=nil;return map[string]interface{}{"orderId":int64(1)},nil}
...
func (f *fakeAIFreeTrader) GetOrderStatus(string,string)(map[string]interface{},error){return map[string]interface{}{"status":"FILLED"},nil}
...
func TestAIFreeAccountTPLocksAndFlattens(t *testing.T) {
	st,err:=store.New(filepath.Join(t.TempDir(),"ai-free.db")); if err!=nil{t.Fatal(err)}; defer st.Close()
	cfg:=store.GetAIFreeStrategyConfig("en")
	cfg.RiskControl.ManagedCapitalUSDT=100
	...
	if _,err:=at.ensureAIFreeSession();err!=nil{t.Fatal(err)}
	if err:=at.checkAIFreeSessionRisk();err!=nil{t.Fatal(err)}
	if len(fake.positions)!=0{t.Fatalf("expected positions flattened, got %+v",fake.positions)}
	latest,err:=at.sessionStore().GetLatest("t1");if err!=nil{t.Fatal(err)}
	if latest==nil||latest.Status!=store.TradingSessionTPLocked{t.Fatalf("expected TP lock, got %+v",latest)}
	if err:=at.aiFreeSessionAllowsOpen();err==nil{t.Fatal("locked session must block new opens")}
}
