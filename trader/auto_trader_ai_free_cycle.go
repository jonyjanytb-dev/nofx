	...
	for {
		...
		case <-timer.C:
			if err := at.runCycle(); err != nil {
				at.logErrorf("❌ ai_free cycle failed: %v", err)
			}
...
func sortAIFreeDecisions(in []kernel.Decision) []kernel.Decision {
	...
	for _, d := range in {
		if d.Action == "close_long" || d.Action == "close_short" {
			closed[strings.ToUpper(d.Symbol)] = true
		}
	}
	out := make([]kernel.Decision, 0, len(in))
	for _, d := range in {
		if (d.Action == "open_long" || d.Action == "open_short") && closed[strings.ToUpper(d.Symbol)] {
			continue
		}
		out = append(out, d)
	}
	...
	sort.SliceStable(out, func(i, j int) bool {
		...
		if pi == 1 && out[i].Confidence != out[j].Confidence {
