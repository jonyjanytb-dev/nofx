...
func sameSymbolPositionExists(positions []map[string]interface{}, symbol string) bool {
	...
	for _, p := range positions {
		...
		if qty != 0 && market.Normalize(raw) == target {
			return true
		}
	}
	return false
...
func validateAIFreeProtectionPrices(action string, marketPrice, sl, tp float64) error {
	if marketPrice <= 0 || sl <= 0 {
		return fmt.Errorf("market price and mandatory stop loss must be positive")
	}
	switch action {
	case "open_long":
		if sl >= marketPrice {
			return fmt.Errorf("long stop must be below market")
		}
		if tp > 0 && tp <= marketPrice {
			return fmt.Errorf("long take profit must be above market")
		}
	case "open_short":
		if sl <= marketPrice {
			return fmt.Errorf("short stop must be above market")
		}
		if tp > 0 && tp >= marketPrice {
			return fmt.Errorf("short take profit must be below market")
		}
	default:
...
func (at *AutoTrader) applyAIFreeOpenRiskBudget(d *kernel.Decision, currentPrice, availableBalance float64, positions []map[string]interface{}) (*aiFreeRiskAdjustment, error) {
	...
	if r.MaxLossPerTradeUSDT <= 0 {
		return nil, fmt.Errorf("max loss per trade must be configured")
	}
	if r.MaxLeverage > 0 && d.Leverage > r.MaxLeverage {
		d.Leverage = r.MaxLeverage
	}
	if d.Leverage <= 0 {
	...
	case "open_long":
		if d.StopLoss >= currentPrice {
			return nil, fmt.Errorf("invalid long stop")
		}
		stopMove = (currentPrice - d.StopLoss) / currentPrice
	case "open_short":
		if d.StopLoss <= currentPrice {
			return nil, fmt.Errorf("invalid short stop")
		}
	...
	if allowed > maxByLoss {
		allowed = maxByLoss
	}
	if allowed > maxByManaged {
		allowed = maxByManaged
	}
	if allowed > maxByBalance {
		allowed = maxByBalance
	}
	if allowed <= 0 {
		return nil, fmt.Errorf("no margin available")
	}
