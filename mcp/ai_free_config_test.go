...
func TestConfigureAIFreeClientDisablesDeepSeekThinking(t *testing.T) {
	...
	if base.Cfg.MaxRetries != 1 {
		t.Fatalf("expected exactly one attempt, got MaxRetries=%d", base.Cfg.MaxRetries)
	}
	if base.HTTPClient.Timeout != 60*time.Second {
		t.Fatalf("expected 60s timeout, got %v", base.HTTPClient.Timeout)
	}
	if base.Cfg.Temperature != 0.2 {
